生产环境使用mongo副本集时,遇到了一个问题,mongo配置了?readPreference=secondary(表示读操作只在从节点),在操作一个Document的增删改时,需要读取数据,填充到另一个Document的数据,因为配置了secondary,主从数据同步未完成,导致读数据未读到,而填充了错误的数据。  
